        ...
        <div className="scan">
          ...
          <div className="scan-line">
            <span>
              ...
              {(status?.scan?.estimatedTotal ?? 0).toLocaleString()} blocks
              replayed
            </span>
            <span>
              {status?.scan?.perSecond
                ? `${status.scan.perSecond.toLocaleString()} blocks/sec · about ${remaining} left`
                : "starting up"}
            </span>
          </div>
          <div className="scan-note">
            Midnight pays transaction fees in DUST, which is generated by
            registered NIGHT and can only be found by replaying the chain. This
            runs once; the scan is then cached to disk, so every later restart
            takes seconds. Nothing on this page is faked while it runs — there
            is simply no contract to read yet.
          </div>
        </div>
      )}
    </section>
